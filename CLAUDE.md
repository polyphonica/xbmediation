...
- **Hosting target is a self-managed IONOS VPS** (Docker Compose for app+db, proxied by the server's existing host-installed nginx — not Dockerized, and not a managed platform) — see `docker-compose.yml`, `Dockerfile`, `nginx/xbmediation.conf`, `deploy/deploy.sh`. The `app` container only publishes to `127.0.0.1` (via `APP_PORT`); nginx is what's actually internet-facing.
