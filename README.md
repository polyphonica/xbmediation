...
- **Hosting:** self-hosted on an IONOS VPS via Docker Compose + Caddy (see `docker-compose.yml`, `Dockerfile`, `Caddyfile`, `deploy/deploy.sh`) — no managed platform (e.g. Vercel) in the loop
...
- A registered domain (site currently assumes `xb-mediation.de` in `Caddyfile` / `.env.example`)

## Deployment

The app is designed to run entirely on a self-managed IONOS VPS:

- `Dockerfile` — multi-stage build, Next.js standalone output
- `docker-compose.yml` — `app` + `db` (Postgres, internal network only) + `caddy` (reverse proxy, automatic HTTPS)
